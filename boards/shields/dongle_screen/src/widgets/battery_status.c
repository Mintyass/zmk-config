...
static lv_color_t battery_color(uint8_t level) {
    if (level < 1) {
        return lv_color_hex(0xC45C4A);
    }
    if (level <= 10) {
        return lv_color_hex(0xE0B15A);
    }
    return lv_color_hex(0x8FCB6A);
}

static void draw_battery(lv_obj_t *canvas, uint8_t level, bool usb_present) {
    lv_color_t color = battery_color(level);
    ...
    outline.radius = 2;
    lv_canvas_draw_rect(canvas, 0, 1, 36, 16, &outline);

    lv_draw_rect_dsc_init(&fill);
    fill.bg_color = color;
    fill.bg_opa = LV_OPA_COVER;
    fill.radius = 1;
    lv_canvas_draw_rect(canvas, 38, 6, 4, 6, &fill);

    if (level > 0) {
        width = (level * inner) / 100;
        if (width < 3) {
            width = 3;
        }
        if (width > inner) {
        ...
        lv_canvas_draw_rect(canvas, 4, 4, width, 10, &fill);
    }
}
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    lv_obj_set_size(widget->obj, 280, 36);
    ...
    for (int i = 0; i < ZMK_SPLIT_CENTRAL_PERIPHERAL_COUNT + SOURCE_OFFSET; i++) {
        ...
        lv_canvas_set_buffer(image_canvas, battery_image_buffer[i], BATT_CANVAS_W, BATT_CANVAS_H,
                             LV_IMG_CF_TRUE_COLOR);

        lv_obj_align(image_canvas, LV_ALIGN_LEFT_MID, 24 + (i * 136), 0);
        lv_obj_align(battery_label, LV_ALIGN_LEFT_MID, 72 + (i * 136), 0);
