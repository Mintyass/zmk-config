...
static struct zmk_widget_caps_status caps_widget;
...
lv_obj_t *zmk_display_status_screen()
{
    ...
    lv_style_set_text_color(&global_style, lv_color_hex(0xC9C3B6));
    ...
    lv_obj_align(zmk_widget_layer_status_obj(&layer_status_widget), LV_ALIGN_LEFT_MID, 10, 8);
    zmk_widget_layer_status_place_fades(&layer_status_widget);
#endif

#if CONFIG_DONGLE_SCREEN_MODIFIER_ACTIVE
    zmk_widget_mod_status_init(&mod_widget, screen);
    lv_obj_align(zmk_widget_mod_status_obj(&mod_widget), LV_ALIGN_RIGHT_MID, -8, 8);
#endif

    zmk_widget_caps_status_init(&caps_widget, screen);
    lv_obj_align(zmk_widget_caps_status_label(&caps_widget), LV_ALIGN_TOP_LEFT, 12, 12);
